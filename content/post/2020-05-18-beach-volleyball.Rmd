...
vb_matches_tidy <- vb_matches %>% 
  mutate(w_attacks = w_p1_tot_attacks + w_p2_tot_attacks,
         w_kills = w_p1_tot_kills + w_p1_tot_kills,
         w_errors = w_p1_tot_errors + w_p2_tot_errors,
         w_aces = w_p1_tot_aces + w_p2_tot_aces,
         w_serve_errors = w_p1_tot_serve_errors + w_p1_tot_serve_errors,
         w_blocks = w_p1_tot_blocks + w_p1_tot_blocks,
         w_digs = w_p1_tot_digs + w_p2_tot_digs,
         l_attacks = l_p1_tot_attacks + l_p2_tot_attacks,
         l_kills = l_p1_tot_kills + l_p1_tot_kills,
         l_errors = l_p1_tot_errors + l_p2_tot_errors,
         l_aces = l_p1_tot_aces + l_p2_tot_aces,
         l_serve_errors = l_p1_tot_serve_errors + l_p1_tot_serve_errors,
         l_blocks = l_p1_tot_blocks + l_p1_tot_blocks,
...
vb_matches_tidy2 <- vb_matches_tidy %>% 
...
The distribution of kills for winning and losing teams are roughly the same shape, but the distribution for winning teams is noticably shifted to the right of the distribution for losing teams. This makes sense because in order for a team to win, they must score more points than the other team, thus winning teams would have more overall kills than losing teams. 

```{r, echo = FALSE}
# kills
vb_matches_tidy4 <- vb_matches_tidy2 %>% 
  filter(win == "w_kills" | win == "l_kills") %>% 
  filter(values != "NA") %>% 
  mutate(win = ifelse(win == "w_kills", "Winning Team", "Losing Team"))

ggplot(vb_matches_tidy4, aes(x = values, fill = win)) +
...
```{r, echo = FALSE}
# filter to only focus on attacks
vb_matches_tidy3 <- vb_matches_tidy2 %>% 
  filter(win == "w_attacks" | win == "l_attacks") %>% 
  filter(values != "NA") %>% 
  mutate(win = ifelse(win == "w_attacks", "Winning Team", "Losing Team"))

# create boxplots showing distribution of attacks for winners versus losers
ggplot(vb_matches_tidy3, aes(x = values, fill = win)) +
  geom_density(alpha = 0.7) + 
  labs(
...
```{r, echo = FALSE}
vb_matches_grouped <- vb_matches_tidy2 %>% 
  filter(values != "NA") %>% 
